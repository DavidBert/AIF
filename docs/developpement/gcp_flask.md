...
Once your instance is created, you can connect to it using the following command:
```console
...
We can make it a little bit nicer by adding a template and a static folder.
Create a new file named `templates/index.html` containing the following code:
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
Modify the `app.py` file to use the template and the static folder:
```python
...
python app.py
```
Go to `http://localhost:5000` to see your web app currently running locally.
...
To do so, run the following command on the instance terminal:
```console
...
If everything is working, you should see your web app.
...
For the sake of simplicity we will make this repository public.  This will allow the remote instance to access the repository without the need for authentication. 
Go to [GitHub](https://github.com/) and create a new public repository named `hello-world-flask-app`.
On the local folder `hello-world-flask-app`, initialize a new repository:
```console
git init
```
